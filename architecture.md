...
- `internal/netutil`：枚举本机私有 IPv4 地址，生成源端同步地址候选。
...
- 创建源端或目标端任务。
- 通过页面内表单填写直连端点和可选 Relay 端点，生成同步链接。
- 生成链接表单可提示源端证书 IP/DNS 名称和本机私有 IPv4 地址加默认同步端口组成的候选端点；默认同步端口由主程序 `-sync-port` 提供。
- 管理页可读取源端直连 TLS 证书是否已加载，并在源端任务和生成链接窗口提示直连就绪状态；生成链接时会提示直连地址是否被源端证书 IP/DNS 名称覆盖。
- 后端配置接口返回源端证书地址候选端点，负责去重、跳过通配符名称，并按需为 IPv6 地址添加方括号。
- 未加载源端 TLS 证书且未填写 Relay 端点时，后端拒绝生成源端链接，避免产生无法启动的直连链接。
...
端点候选接口只返回 RFC1918 私有 IPv4 地址，不返回回环地址、公网地址、IPv6 地址或虚拟测试网段地址；候选端口默认是 `7443`，可通过 `-sync-port` 调整；候选仅用于辅助填写，最终端点仍由用户确认。
...
- 未加载 TLS 证书时，管理页提示直连不会监听；配置 Relay 的源端任务仍可通过 Relay 等待目标端连接。
...
15. `feature/link-form-ui`
    - 将生成链接流程从浏览器 prompt 改为管理页表单。
16. `feature/endpoint-suggestions`
    - 在生成链接表单中推荐本机私有 IPv4 同步地址候选。
17. `feature/sync-port-config`
...
20. `feature/cert-host-warning`
    - 在生成链接窗口提示源端证书是否覆盖所填直连地址。
21. `feature/cert-endpoint-suggestions`
    - 在生成链接窗口优先展示源端证书覆盖的直连地址候选。
22. `feature/config-certificate-endpoints`
    - 后端配置接口返回经过校验和格式化的源端证书端点候选。
23. `feature/reject-unusable-source-links`
    - 拒绝生成既无源端 TLS 证书也无 Relay 端点的不可用源端链接。
24. `feature/web-regression-tests`
...
31. `feature/auto-start-after-link`
    - 源端生成链接后自动启动，目标端加入链接后自动启动，减少验收操作步骤。
32. `feature/embed-source-cert-in-link`
   - 源端同步链接携带公开证书，目标端一次粘贴即可建立直连信任。
33. `feature/auto-detect-source-hosts`
   - 验收脚本自动检测源端局域网 IPv4，减少手工编辑证书地址。
34. `feature/source-link-restart-and-task-ui`
   - 同步任务列表作为主界面，增加清晰运行状态、源端最近链接复用入口和任务删除能力。
35. `feature/automatic-source-tls`
...
49. `feature/settings-stats-and-session-diagnostics`
   - 左侧设置模块展示当前版本、管理页地址、同步端口、同步间隔、数据目录和源端 TLS 状态；任务主表增加整体统计汇总；客户端任务日志记录与 Relay 一致的会话摘要，便于跨机器排查连接问题。
50. `feature/relay-access-token`
   - Relay 服务支持服务器访问令牌；生成同步链接时可填写 Relay TLS 地址和 Relay 令牌，链接携带 Relay 公开证书和访问令牌，目标端只需粘贴同步链接即可通过 Relay 连接。
51. `feature/deploy-logs-devices-web-tests`
   - Linux 客户端和 Relay 部署脚本支持直接传入 `PACKAGE_URL`，在 GitHub API、raw 缓存或代理失效时仍可安装指定验收包。
   - 日志窗口支持按级别和关键词筛选，并可复制或下载当前可见日志。
   - 设备管理增加全局设备汇总、信任标记、设备历史和单任务设备历史查看。
   - 新增前端行为测试，用 Node 执行真实管理页脚本，覆盖日志筛选和全局设备汇总逻辑。
52. `feature/upgrade-windows-diagnostics-stability`
   - Linux 客户端和 Relay 的 `upgrade` 命令支持 `RELEASE_TAG` 与 `PACKAGE_URL`，升级流程不再强依赖 GitHub API。
   - OneSync 默认把服务日志写入数据目录，Windows 直接双击 exe 也能留下可收集日志。
   - 管理页提供诊断 zip 下载，打包诊断文本和服务日志尾部，便于测试机提交问题。
   - Windows 验收包新增打开管理页、打开日志目录和收集诊断包的辅助脚本。
   - 同步引擎增加源端权威但不删除目标端多余文件的回归测试，验收模板补充多文件、大文件、Relay 重启和诊断包证据。
53. `feature/release-versioning`
   - 发布版本从 `1.00` 开始递增，GitHub Release tag 使用 `v1.00`、`v1.01` 这种稳定格式。
   - Windows/Linux 包名使用发布版本号，不再使用提交短号；构建提交号仍保留在 `BUILD.txt` 和 `MANIFEST.txt` 中。
   - 程序内显示版本使用发布版本号，便于用户截图、日志和诊断包对齐。
   - Linux 安装和升级脚本支持 `RELEASE_TAG=v1.00` 格式，同时保留旧 `acceptance-*` 包兼容逻辑。
