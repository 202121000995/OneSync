...
ACTION=${1:-install}
GH_PROXY=${GH_PROXY:-}
DEFAULT_RELEASE_TAG=${ONESYNC_DEFAULT_RELEASE_TAG:-v1.40}
...
install_relay() {
	if [ -z "${RELAY_HOSTS:-}" ]; then
		...
		printf '  curl -fsSL https://gh-proxy.org/https://raw.githubusercontent.com/%s/main/packaging/acceptance-scripts/linux/deploy-relaytls.sh | sudo env RELAY_HOSTS=relay.example.com RELAY_PORT=443 RELAY_TOKEN=your-secret RELEASE_TAG=v1.40 GH_PROXY=https://gh-proxy.org/ sh\n' "$REPO" >&2
	...
	if [ -z "$url" ]; then
		...
		printf 'If GitHub API is blocked by the proxy, retry with RELEASE_TAG=v1.40 or PACKAGE_URL=https://...tar.gz.\n' >&2
		exit 1
	fi
