...
```sh
sudo env RELEASE_TAG=v1.38 GH_PROXY=https://gh-proxy.org onesyncctl upgrade
sudo env PACKAGE_URL=https://gh-proxy.org/https://github.com/202121000995/OneSync/releases/download/v1.38/onesync-linux-amd64-v1.38.tar.gz onesyncctl upgrade
```
...
```sh
curl -fsSL https://gh-proxy.org/https://raw.githubusercontent.com/202121000995/OneSync/main/packaging/acceptance-scripts/linux/deploy-onesync.sh | sudo env RELEASE_TAG=v1.38 GH_PROXY=https://gh-proxy.org/ sh
```

If the GitHub API or raw script cache is blocked, pass the Linux package URL directly:

```sh
curl -fsSL https://gh-proxy.org/https://raw.githubusercontent.com/202121000995/OneSync/main/packaging/acceptance-scripts/linux/deploy-onesync.sh | sudo env PACKAGE_URL=https://gh-proxy.org/https://github.com/202121000995/OneSync/releases/download/v1.38/onesync-linux-amd64-v1.38.tar.gz sh
```
...
```sh
curl -fsSL https://gh-proxy.org/https://raw.githubusercontent.com/202121000995/OneSync/main/packaging/acceptance-scripts/linux/deploy-relaytls.sh | sudo env RELAY_HOSTS=relay.example.com RELAY_PORT=443 RELAY_TOKEN=your-secret RELEASE_TAG=v1.38 GH_PROXY=https://gh-proxy.org/ sh
```

If the GitHub API or raw script cache is blocked, pass the Linux package URL directly:

```sh
curl -fsSL https://gh-proxy.org/https://raw.githubusercontent.com/202121000995/OneSync/main/packaging/acceptance-scripts/linux/deploy-relaytls.sh | sudo env RELAY_HOSTS=relay.example.com RELAY_PORT=443 RELAY_TOKEN=your-secret PACKAGE_URL=https://gh-proxy.org/https://github.com/202121000995/OneSync/releases/download/v1.38/onesync-linux-amd64-v1.38.tar.gz sh
```
...
```sh
curl -fsSL https://gh-proxy.org/https://raw.githubusercontent.com/202121000995/OneSync/main/packaging/acceptance-scripts/linux/deploy-relaytls.sh | sudo env RELAY_HOSTS=relay.example.com RELAY_PORT=443 RELAY_TOKEN=your-secret ONESYNC_RELAY_CERT=/path/fullchain.pem ONESYNC_RELAY_KEY=/path/privkey.pem RELEASE_TAG=v1.38 GH_PROXY=https://gh-proxy.org/ sh
```

After Relay deployment, use `onesyncr` to view the Chinese Relay command menu.

Relay also starts a small admin panel by default:

```text
http://server-ip:8766
```

The first visit asks you to set the admin username and password. The panel can show Relay status, sessions, traffic, and logs; create, disable, and delete multiple customer Relay tokens; view and rotate the legacy Relay access token; show certificate information; save BT Panel / 1Panel certificate paths; paste certificate PEM plus private key text directly; and restart the Relay service. To change the panel listen address during install, pass `ONESYNC_RELAY_ADMIN_LISTEN=0.0.0.0:8766`.
...
```sh
sudo env RELEASE_TAG=v1.38 GH_PROXY=https://gh-proxy.org onesync-relayctl upgrade
sudo env PACKAGE_URL=https://gh-proxy.org/https://github.com/202121000995/OneSync/releases/download/v1.38/onesync-linux-amd64-v1.38.tar.gz onesync-relayctl upgrade
```
...
Run `sudo onesync-relayctl cert-info` to check the current certificate path, validity dates, and whether the private key matches the certificate. Run `sudo onesync-relayctl set-cert /path/fullchain.pem /path/privkey.pem` to switch Relay to a certificate managed by BT Panel, 1Panel, or another ACME client. Run `sudo onesync-relayctl set-ports 17443 8766` to change the Relay port and admin panel port.

Run `sudo onesync-relayctl info` to print all text needed by the source link form: Relay address, Relay token, and Relay certificate PEM. If the Relay domain or IP changes, regenerate the certificate with `sudo RELAY_HOSTS=relay.example.com RELAY_PORT=443 onesync-relayctl regen-cert`.
Run `sudo onesync-relayctl rotate-token` to rotate the Relay access token and restart Relay. Existing links that carry the old Relay token must be regenerated.
